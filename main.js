...
var GraphClient = class {
  ...
  async listTasks(listId, limit = 200, onlyActive = false) {
    var _a;
    const base2 = `https://graph.microsoft.com/v1.0/me/todo/lists/${encodeURIComponent(listId)}/tasks`;
    const withFilter = `${base2}?$top=50${onlyActive ? `&$filter=status ne 'completed'` : ""}`;
    let url = withFilter;
    const tasks = [];
...
var MicrosoftToDoLinkPlugin = class extends import_obsidian.Plugin {
  ...
  async syncCurrentFileNow() {
    ...
    try {
      ...
      for (const listId of listIds) {
        ...
        let headingText = void 0;
        if (listIds.length > 1) {
          const name = listMap.get(listId) || "List";
          suffixTag = this.buildListTagSuffix(name);
          headingText = this.settings.pullGroupUnderHeading ? this.buildListHeadingText(name) : void 0;
        }
  ...
  async pullTodoTasksIntoFile(file, listId, syncAfter, tagSuffix, headingTextOverride) {
    await this.getValidAccessToken();
    ...
    if (tagSuffix && tagForPull) {
      ...
      const base2 = this.settings.pullAppendTagEnabled ? this.settings.pullAppendTag : "MicrosoftTodo";
      tagForPull = `${base2}${tagSuffix}`;
    }
    const insertLines = [];
    ...
    for (const task of newTasks) {
      ...
      try {
        ...
        for (const item of items) {
          ...
          const childLine = `  - [${item.isChecked ? "x" : " "}] ${buildMarkdownTaskText(displayName, void 0, tagForPull)} ${buildSyncMarker(childBlockId, this.settings.syncMarkerFormat)}`;
  ...
  async pullChecklistIntoFile(file, listId, tagSuffix) {
    ...
    for (const parent of parents) {
      ...
      for (const item of remoteItems) {
        ...
        const childBlockId = `${CHECKLIST_BLOCK_ID_PREFIX}${randomId(8)}`;
        toInsert.push(`  - [ ] ${buildMarkdownTaskText(name, void 0, tagForPull)} ${buildSyncMarker(childBlockId, this.settings.syncMarkerFormat)}`);
  ...
  async syncFileTwoWay(file) {
    ...
    const listNameById = /* @__PURE__ */ new Map();
    if (this.todoListsCache.length > 0) {
      for (const id of listIds) {
        const name = (_a = this.todoListsCache.find((l) => l.id === id)) == null ? void 0 : _a.displayName;
        if (name) listNameById.set(id, name);
      }
    }
    const tagBases = this.getPullTagNamesToPreserve();
    const listTagToId = /* @__PURE__ */ new Map();
    for (const [id, name] of listNameById) {
      const suffix = this.buildListTagSuffix(name);
      if (!suffix) continue;
      for (const base2 of tagBases) {
        listTagToId.set(`#${base2}${suffix}`, id);
      }
    }
    for (const base2 of tagBases) {
      const tag = `#${base2}`;
    ...
    for (const task of tasks) {
      const parentBlockId = (_b = parentByBlockId.get(task.blockId)) != null ? _b : null;
      if (parentBlockId) {
        let currentParentId = parentBlockId;
        while (currentParentId && currentParentId.startsWith(CHECKLIST_BLOCK_ID_PREFIX)) {
          currentParentId = (_c = parentByBlockId.get(currentParentId)) != null ? _c : null;
        }
        ...
        if (!localChanged2 && graphChanged2) {
          const updatedLine = `${task.indent}${task.bullet} [${remote2.isChecked ? "x" : " "}] ${buildMarkdownTaskText(remote2.displayName, void 0, task.mtdTag)} ${buildSyncMarker(task.blockId, this.settings.syncMarkerFormat)}`;
      ...
      if (localChanged && !graphChanged) {
        await this.graph.updateTask(existing.listId, existing.graphTaskId, task.title, task.completed, (_d = task.dueDate) != null ? _d : null);
        ...
        this.dataModel.taskMappings[mappingKey] = {
          ...
          lastSyncedFileMtime: fileMtime,
          lastKnownGraphLastModified: (_e = latest == null ? void 0 : latest.lastModifiedDateTime) != null ? _e : remote.lastModifiedDateTime
        };
        continue;
      }
      if (!localChanged && graphChanged) {
      ...
      if (graphTime > localTime) {
        ...
        await this.graph.updateTask(existing.listId, existing.graphTaskId, task.title, task.completed, (_f = task.dueDate) != null ? _f : null);
        const latest = await this.graph.getTask(existing.listId, existing.graphTaskId);
        const latestGraphHash = latest ? hashGraphTask(latest) : graphHash;
        this.dataModel.taskMappings[mappingKey] = {
          ...existing,
          ...
          lastSyncedGraphHash: latestGraphHash,
          lastSyncedFileMtime: fileMtime,
          lastKnownGraphLastModified: (_g = latest == null ? void 0 : latest.lastModifiedDateTime) != null ? _g : remote.lastModifiedDateTime
        };
      }
    }
  ...
  buildListTagSuffix(listName) {
    const safeName = (listName || "").replace(/[\s\W]+/g, "-").replace(/^-+|-+$/g, "");
    if (!safeName) return void 0;
    return `-${safeName}`;
  }
  buildListHeadingText(listName) {
    const base2 = (this.settings.pullHeadingText || DEFAULT_SETTINGS.pullHeadingText).trim() || DEFAULT_SETTINGS.pullHeadingText;
    return `${base2} - ${listName}`;
  }
  resolveListIdForTask(task, listTagToId, defaultListId) {
    if (task.mtdTag) {
  ...
  resolvePullInsertIndex(lines, file, headingTextOverride) {
...
function buildSyncMarker(blockId, format) {
...
function ensureBlockIds(lines, tasks, format = "html") {
  let changed = false;
  const updated = [];
  const stack = [];
  for (const task of tasks) {
...
function formatTaskLine(task, title, completed, dueDate, format = "html") {
...
function sanitizeTitleForGraph(title) {
  ...
  if (!input) return "";
  const withoutIds = input.replace(/\^mtdc?_[a-z0-9_]+/gi, " ").replace(/<!--\s*(?:mtd|MicrosoftToDoSync)\s*:\s*[a-z0-9_]+\s*-->/gi, " ").replace(/%%\s*(?:mtd|MicrosoftToDoSync)\s*:\s*[a-z0-9_]+\s*%%/gi, " ").replace(/\s{2,}/g, " ").trim();
  return withoutIds;
}
function buildMarkdownTaskText(title, dueDate, tag) {
  const trimmedTitle = (title || "").trim();
  if (!trimmedTitle) return trimmedTitle;
  const base2 = dueDate ? `${trimmedTitle} \u{1F4C5} ${dueDate}` : trimmedTitle;
  const normalizedTag = (tag || "").trim();
  if (!normalizedTag) return base2;
